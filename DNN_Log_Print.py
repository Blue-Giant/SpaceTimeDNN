...
def dictionary2space_time(R_dic, log_fileout):
    ...
    if (R_dic['train_model']) == 'union_training':
        ...
        DNN_tools.log_string('The model for training loss: %s\n' % 'total loss + loss_it', log_fileout)
    elif (R_dic['train_model']) == 'group3_training':
        DNN_tools.log_string('The model for training loss: %s\n' % 'total loss + loss_it + loss_init_bd', log_fileout)
    elif (R_dic['train_model']) == 'group4_training':
        DNN_tools.log_string('The model for training loss: %s\n' % 'total loss + loss_bd + loss_it + loss_init', log_fileout)
...
def print_and_log_train_one_epoch2space_time(i_epoch, run_time, tmp_lr, temp_penalty_bd, pwb, loss_it_tmp, loss_bd_tmp,
